...
serve(async (req) => {
  try {
    ...
    // IMPORTANT: Fallback if collection_method is missing to prevent crashes
    const collectionMethod = body.collection_method || "Self Collection";
          ...
          <div style="padding: 30px;">
            ...
            <div style="margin: 25px 0; border: 1px solid #e5e7eb; border-radius: 8px; padding: 20px; background-color: #fafafa;">
              <table style="width: 100%;">
                ...
                <tr>
                  <td style="color: #71717a; font-size: 14px; padding: 5px 0;">Collection</td>
                  <td style="text-align: right; font-weight: bold; font-size: 14px;">${collectionMethod}</td>
                </tr>
              </table>
            </div>
    ...
    const adminHtml = `
      <div style="font-family: sans-serif; padding: 20px;">
        <div style="border-left: 4px solid ${accentColor}; padding-left: 15px;">
          ...
          <p style="color: #666; font-size: 14px;">A new appointment has been scheduled.</p>
        </div>
        <div style="margin-top: 20px; background: #f8fafc; padding: 15px; border-radius: 5px;">
          ...
          <p><b>Collection Via:</b> ${collectionMethod}</p>
          <p><b>Contact:</b> ${body.email}</p>
          <p><b>Notes:</b> ${body.notes || "None"}</p>
        </div>
      </div>
    `;
    ...
    await Promise.all([
      ...
      transporter.sendMail({
        ...
        subject: `🚨 New Booking: ${body.full_name} (${collectionMethod})`,
        html: adminHtml,
      })
    ]);
